...
export function DateTimeRangePicker({ onChange, onOk, onError, format: dateFormat = "yyyy-MM-dd HH:mm", showTime = { format: "HH:mm" }, className, minDate, maxDate, initialValue = null, }) {
    // 状态管理
    ...
    const selectedRange = dateRange.start.date && dateRange.end.date
        ...
        : undefined;
    return (_jsx("div", { className: cn("grid gap-2", className), children: _jsxs(Popover, { open: isOpen, onOpenChange: setIsOpen, children: [_jsx(PopoverTrigger, { asChild: true, children: _jsxs(Button, { variant: "outline", className: cn("w-full justify-start text-left font-normal transition-all hover:bg-muted/50", !dateRange.start.date && "text-muted-foreground"), children: [_jsx(CalendarIcon, { className: "mr-2 h-4 w-4 text-primary" }), _jsx("span", { className: "truncate", children: displayText })] }) }), _jsx(PopoverContent, { className: "w-auto p-0 shadow-lg rounded-lg border-0 overflow-hidden", align: "start", children: _jsxs("div", { className: "p-4 bg-background border rounded-lg", children: [error && (_jsx("div", { className: "mb-3 p-2 bg-destructive/10 text-destructive text-sm rounded-md", children: error })), _jsx(Calendar, { mode: "range", locale: zhCN, selected: selectedRange, onSelect: handleDateChange, className: "rounded-md border mb-3 shadow-sm w-full" }), showTime && (_jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 p-3 bg-muted/30 rounded-md", children: [_jsxs("div", { className: "grid gap-2", children: [_jsx(Label, { htmlFor: "start-time", className: "text-sm font-medium", children: "\u5F00\u59CB\u65F6\u95F4" }), _jsx(Input, { id: "start-time", type: "time", value: dateRange.start.time, onChange: (e) => setDateRange((prev) => (Object.assign(Object.assign({}, prev), { start: Object.assign(Object.assign({}, prev.start), { time: e.target.value }) }))), className: "w-full min-w-[120px] border-muted-foreground/20 focus:border-primary transition-all" })] }), _jsxs("div", { className: "grid gap-2", children: [_jsx(Label, { htmlFor: "end-time", className: "text-sm font-medium", children: "\u7ED3\u675F\u65F6\u95F4" }), _jsx(Input, { id: "end-time", type: "time", value: dateRange.end.time, onChange: (e) => setDateRange((prev) => (Object.assign(Object.assign({}, prev), { end: Object.assign(Object.assign({}, prev.end), { time: e.target.value }) }))), className: "w-full min-w-[120px] border-muted-foreground/20 focus:border-primary transition-all" })] })] })), _jsxs("div", { className: "flex justify-end gap-3 mt-5 pt-3 border-t border-border", children: [_jsx(Button, { variant: "ghost", onClick: () => {
                                            ...
                                            // 清除错误状态
                                            setError(null);
                                        }, children: "\u6B64\u523B (\u8DF3\u8F6C\u5230\u4ECA\u5929)" }), _jsx(Button, { variant: "ghost", onClick: () => {
                                            resetDateRange();
                                        }, className: "hover:bg-muted transition-colors", children: "\u91CD\u7F6E" }), _jsx(Button, { variant: "secondary", onClick: () => setIsOpen(false), className: "hover:bg-muted transition-colors", children: "\u53D6\u6D88" }), _jsx(Button, { onClick: handleOk, className: "bg-primary hover:bg-primary/90 transition-colors", children: "\u786E\u5B9A" })] })] }) })] }) }));
}
